...
if __name__ == "__main__":
    ...
    # Configuration tracking variables updated to process spaghetti grouping matrices
    num_groups = 30
    ...
    try:
        ...
        # Execute macro groupings sequentially while running concurrent threads inside each group
        global_counter = 0
        for g_idx in range(num_groups):
            ...
            # Safely capture the mean vector of the completed macro group
            if len(current_group_data) > 0:
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", category=RuntimeWarning)
                    # Convert any embedded None values to np.nan to avoid TypeError during reduction
        ...
        # Plot structural flight segments under the curves
        ...
        # Draw the thin, faint red spaghetti paths representing macro group variance
        for idx, g_mean in enumerate(group_means):
            label = 'Macro-Group Means (100 runs each)' if idx == 0 else ""
            plt.plot(k_vals, g_mean, color='red', alpha=0.15, linewidth=1.0, label=label)
            
        # Draw the thick Grand Mean tracking line across all cumulative runs
        plt.plot(k_vals, grand_mean, color='darkred', linewidth=2.5, label='Grand Empirical Mean (3,000 runs total)')
        
        # Draw variance envelope with standard lower boundary clipped firmly at 0ms
        plt.fill_between(k_vals, np.maximum(0, grand_mean - grand_std), grand_mean + grand_std, color='lightcoral', alpha=0.12, label='Total Trial Jitter (±1 STD)')
        
        # Plot the smooth baseline theoretical fluid model line
        ...
        plt.title(f'TCP Segment Delivery Times ({num_groups} Groups × {trials_per_group} Trials Spaghetti Visualization)', fontsize=14)
        ...
        plt.tight_layout()
        plot_file = 'tcp_spaghetti_validation_plot.png'
