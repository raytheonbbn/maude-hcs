...
Parses network topology, TGEN traffic profiles, loss profiles, and parameters
dynamically from the scenario YAML configuration.
...
def parse_scenario_yaml(yaml_path):
    ...
    for node_type, n_info in nodes_data.items():
        channel_raw = n_info.get("channel_config", [{}])[0].get("channel", "")
    ...
    return duration, networks, net_id_map, net_short, loss_profiles, hcs_nodes, hcs_profiles_by_channel, tgen_defs
...
def generate_all_tgen_instances(tgen_defs, net_id_map, net_short):
    ...
    for tgen_type, tgen_networks in tgen_defs.items():
        for network, (qty, profiles) in tgen_networks.items():
            net_id = net_id_map.get(network, "srvN")
...
# ═══════════════════════════════════════════════════════════════════════════════
def gen_addresses_file(tgen_instances, net_id_map):
    lines = []
    L = lines.append
    
    L("--- MAUDE_HCS: this file is autogenerated for CP3 Scenario 1 ---")
    L("--- Address mapping: readable names → structured addresses")
    L("--- cl[N] refers to the NetId of the client network N")
    L("")
    L("mod SCENARIO1-ADDRESSES is")
    ...
    # Webtunnel on client_net_racetunnel
    rt_net = net_id_map.get("client_net_racetunnel", "cl[1]")
    for i, idx in [(1, 1), (2, 2)]:
        L(f"  --- Webtunnel Client {i} (Network {rt_net}, index {idx})")
    ...
    # Skyhook on client_net_sky
    sky_net = net_id_map.get("client_net_sky", "cl[2]")
    for i, idx in [(3, 1), (4, 2)]:
        L(f"  --- Skyhook Client {i} (Network {sky_net}, index {idx})")
    ...
    # OBFS4 on client_net_obfs
    obfs_net = net_id_map.get("client_net_obfs", "cl[3]")
    for i, idx in [(5, 1), (6, 2)]:
        L(f"  --- OBFS4 Client {i} (Network {obfs_net}, index {idx})")
    ...
    # Iodine on client_net_iodine
    iod_net = net_id_map.get("client_net_iodine", "cl[4]")
    for i, idx in [(7, 1), (8, 2)]:
        L(f"  --- Iodine Client {i} (Network {iod_net}, index {idx})")
        ...
        L(f"  ops iodCl{i}NetServerAddr : -> Address .")
    L("")
    
    # Mastodon HCS on client_net_mastodon
    mas_net = net_id_map.get("client_net_mastodon", "cl[5]")
    i = 9
    idx = 1
    L(f"  --- Mastodon HCS Client {i} (Network {mas_net}, index {idx})")
    ...
    L("  eq minioS3SrvAddr   = a(srvN, tgen, min, srv, 0) .")
...
# ═══════════════════════════════════════════════════════════════════════════════
def gen_main_file(tgen_instances, networks, loss_profiles, hcs_profiles_by_channel, duration):
    lines = []
    L = lines.append
    
    L("--- MAUDE_HCS: CP3 Scenario 1 Experiment ---")
    ...
    L(f"sload {lib}/raceboatMastodon/maude/user_models/client_config_mastodon_bidi")
    L(f"sload {lib}/raceboatMastodon/maude/user_models/server_config_mastodon_bidi")
    L(f"sload {lib}/common/maude/http-overhead.maude")
    L(f"sload {lib}/cp3-tests/skyhook/skyhook-um-mamodel-1")
    ...
    L(f"sload {lib}/tgen/maude/dnsTgen-actor.maude")
    ...
    L("--- Address mapping")
    L("sload scenario1_addresses")
    L("")
    
    L("mod SCENARIO1-EXPERIMENT is")
    L("  pr SCHEDULER .")
    L("  pr USER-ACTION-ACTOR .")
    L("  pr SKYHOOK-UM-MAMODEL-1 .")
    L("  pr IRC-V2 .")
    ...
    L("  inc MASTODON .")
    L("  inc MASTODON-CLIENT-CONFIG-MASTODON-BIDI-MAMODEL .")
    L("  inc MASTODON-SERVER-CONFIG-MASTODON-BIDI-MAMODEL .")
    L("  pr SKYHOOK .")
    L("  pr S3_PROTOCOL .")
    L("  pr IRC_MONITOR .")
    L("  pr OBFS4 .")
    ...
    L("    < 't1 . 'pwnd . 'com . root, ns, 360000.0, 'ns . 't1 . 'pwnd . 'com . root >")
    L("    < 'ns . 't1 . 'pwnd . 'com . root, a, 360000.0, iodCl7ServerAddr >")
    ...
    for i in [7, 8]:
        L(f"  op zonePwndCom{i} : -> List{{Record}} .")
        L(f"  eq zonePwndCom{i} =")
        L(f"    < 'pwnd . 'com . root, soa, 360000.0, soaData(360000.0) >")
        L(f"    < 'pwnd . 'com . root, ns, 360000.0, 'ns . 'pwnd . 'com . root >")
        L(f"    < 'ns . 'pwnd . 'com . root, a, 360000.0, iodCl{i}ServerAddr >")
        L(f"    < 'www0 . 'pwnd . 'com . root, a, 360000.0, 2 . 0 . 1 . 2 >")
        L(f"    < 'www1 . 'pwnd . 'com . root, a, 360000.0, 2 . 1 . 1 . 2 >")
        L(f"    < wildcard . 'pwnd . 'com . root, txt, 360000.0, nullAddr > .")
        L("")
    ...
    L("    cacheEntry(< 't1 . 'pwnd . 'com . root, ns, 360000.0, 'ns . 't1 . 'pwnd . 'com . root >, 1)")
    L("    cacheEntry(< 'ns . 't1 . 'pwnd . 'com . root, a, 360000.0, iodCl7ServerAddr >, 1)")
    for zone_label, dns_addr in [
        ("mas", "corpMasDnsAddr"), ("rt", "corpRtDnsAddr"),
        ("iod", "corpIodDnsAddr"), ("obfs", "corpObfsDnsAddr"),
        ("sky", "corpSkyDnsAddr"), ("serv", "servDnsAddr")
    ...
    for net_name, (latency_ms, loss_profile_name) in networks.items():
        lp = loss_profiles.get(loss_profile_name, {"p13": 0.0, "p31": 1.0, "p32": 0.0, "p23": 0.0, "p14": 0.0})
        one_way_delay = latency_ms / 1000.0
        op_name = link_param_name(net_name)
        L(f"  op {op_name} : -> AttributeSet .")
        L(f"  eq {op_name} =")
        L(f"  (4stateLoss:")
        L(f"     (p13: {lp['p13']},")
        L(f"      p31: {lp['p31']},")
        ...
        L(f"      p23: {lp['p23']},")
        L(f"      p14: {lp['p14']},")
        L(f"      oneWayDelay: {one_way_delay})")
        L(f"  )")
        L(f"  .")
        L("")
    
    L(f"  op {ixp_link_param_name()} : -> AttributeSet .")
    L(f"  eq {ixp_link_param_name()} =")
    L(f"  (4stateLoss:")
    L(f"     (p13: 0.0,")
    L(f"      p31: 1.0,")
    L(f"      p32: 0.0,")
    L(f"      p23: 0.0,")
    L(f"      p14: 0.0,")
    L(f"      oneWayDelay: 0.005)")
    L(f"  )")
    L(f"  .")
    L("")
    
    L("  op LinkType-Dns : -> AttributeSet .")
    L("  eq LinkType-Dns =")
    L("  (delayStd:  0.0),")
    ...
    tcp_addrs.append("masCl9McacAddr")
    tcp_addrs.append("masCl9McasAddr")
    ...
    tcp_addrs.append("minioS3SrvAddr")
    ...
    for name in udp_addrs:
        L(f"  eq transport({name}) = udp({name}) .")
    L("")
    
    for i in [7, 8]:
        L(f"  eq transport(corpIodDnsAddr)    = corpIodDnsAddr .")
    ...
    def add_bidir_link(addr1, addr2, link_type):
    ...
    for i in [7, 8]:
        ...
        add_bidir_link(f"corpIodDnsAddr", "publicDnsAddr", iod_link)
    ...
    for srv_name in ["ftpSrvAddr", "gorillaSrvAddr", "minioS3SrvAddr"]:
    ...
    for idx, i in enumerate([1, 2]):
        ...
        L(f"  eq wtCl{i}Um        = mkIrcUMV2Actor(wtCl{i}UmAddr, \"{irc_key}\", wtCl{i}IrcAddr) .")
    ...
    for idx, i in enumerate([3, 4]):
        ...
        L(f"  eq skyCl{i}Um        = mkIrcUMV2Actor(skyCl{i}UmAddr, \"{irc_key}\", skyCl{i}IrcAddr) .")
        L(f"  eq skyCl{i}Iface     = mkIrcByteSeqIface(skyCl{i}IfaceAddr, skyCl{i}IrcAddr, skyCl{i}CmacAddr) .")
        L(f"  eq skyCl{i}UmacAct   = mkUMactor(skyCl{i}UmacAddr, skyhook-um-mamodel-1-ma, skyCl{i}CmacAddr) .")
        ...
        L(f"  eq skyCl{i}UmasAct   = mkUMactor(skyCl{i}UmasAddr, skyhook-um-mamodel-1-ma, skyCl{i}CmasAddr) .")
    ...
    for idx, i in enumerate([5, 6]):
        ...
        L(f"  eq obfsCl{i}Um       = mkIrcUMV2Actor(obfsCl{i}UmAddr, \"{irc_key}\", obfsCl{i}IrcAddr) .")
    ...
    for idx, i in enumerate([7, 8]):
        ...
        L(f"  ops iodCl{i}NetSrv : -> Actor .")
        
        L(f"  eq iodCl{i}Irc       = mkIrcClient-v2(iodCl{i}IrcAddr, iodCl{i}IfaceAddr, \"IodClient{i}\") .")
        L(f"  eq iodCl{i}Um        = mkIrcUMV2Actor(iodCl{i}UmAddr, \"{irc_key}\", iodCl{i}IrcAddr) .")
        ...
        L(f"  eq iodCl{i}Cl        = makeWClient(iodCl{i}ClientAddr, corpIodDnsAddr, 't1 . 'pwnd . 'com . root, a, 0.0) .")
        L(f"  eq iodCl{i}Srv       = makeWNameServer(iodCl{i}ServerAddr, 0.0, zonePwndCom{i}) .")
        L(f"  eq iodCl{i}NetSrv    = makeNetClient(iodCl{i}NetServerAddr,")
        L(f"                                       publicDnsAddr,")
    ...
    L(f"  eq masCl{i}Um        = mkIrcUMV2Actor(masCl{i}UmAddr, \"{irc_key}\", masCl{i}IrcAddr) .")
    L(f"  eq masCl{i}Iface     = mkIrcByteSeqIface(masCl{i}IfaceAddr, masCl{i}IrcAddr, masCl{i}CmacAddr) .")
    L(f"  eq masCl{i}UmacAct   = mkUMactor(masCl{i}UmacAddr, mastodon-client-config-mastodon-bidi-ma, masCl{i}CmacAddr) .")
    ...
    L(f"  eq masCl{i}UmasAct   = mkUMactor(masCl{i}UmasAddr, mastodon-server-config-mastodon-bidi-ma, masCl{i}CmasAddr) .")
    ...
    L("  eq s3SrvAct       = makeS3Server(s3SrvAddr) .")
    ...
    L("  eq corpMasDns   = mkNameServer(corpMasDnsAddr, (zoneMasCom), publicDnsAddr) .")
    L("  eq corpRtDns    = mkNameServer(corpRtDnsAddr, (zoneRtCom), publicDnsAddr) .")
    L("  eq corpIodDns   = mkNameServer(corpIodDnsAddr, (zoneIodCom), publicDnsAddr) .")
    L("  eq corpObfsDns  = mkNameServer(corpObfsDnsAddr, (zoneObfsCom), publicDnsAddr) .")
    L("  eq corpSkyDns   = mkNameServer(corpSkyDnsAddr, (zoneSkyCom), publicDnsAddr) .")
    L("  eq servDns       = mkNameServer(servDnsAddr, (zoneServCom), publicDnsAddr) .")
    ...
    for name, dns_addr, net_cl_addr in [
        ("corpMasNetCl", "corpMasDnsAddr", "corpMasNetClAddr"),
        ("corpRtNetCl", "corpRtDnsAddr", "corpRtNetClAddr"),
        ("corpIodNetCl", "corpIodDnsAddr", "corpIodNetClAddr"),
        ("corpObfsNetCl", "corpObfsDnsAddr", "corpObfsNetClAddr"),
        ("corpSkyNetCl", "corpSkyDnsAddr", "corpSkyNetClAddr"),
        ("servNetCl", "servDnsAddr", "servNetClAddr"),
    ]:
        L(f"  eq {name} = makeNetClient({net_cl_addr},")
        L(f"                            publicDnsAddr,")
        L(f"                            {dns_addr},")
        L(f"                            true) .")
    L("  eq publicResolverNetCl = makeNetClient(publicResolverNetClAddr,")
    L("                                        publicDnsAddr,")
    L("                                        publicDnsAddr,")
    L("                                        true) .")
    L("")
    ...
    L("  ops minioS3SrvAct : -> Actor .")
    L("  eq minioS3SrvAct =")
    L("    < minioS3SrvAddr : AwsS3HttpServer |")
    L("        s3DataMap: (")
    ...
    for inst in tgen_instances:
        ...
        if inst.tgen_type == "ftpTgen":
            ...
            L(f"  eq {bn}S3ClAct  = makeS3Client({bn}S3ClAddr, minioS3SrvAddr) .")
            ...
            L(f"  eq {bn}UmAct  = mkIrcUMV2Actor({bn}UmAddr, \"{irc_key}\", {bn}TgAddr) .")
            L(f"  eq {bn}NetClAct = makeNetClient({bn}NetClAddr,")
            L(f"                                  ircServerAddr,")
            L(f"                                  {bn}TgAddr,")
            L(f"                                  true,")
    ...
    for i in [3, 4]:
        L(f"    [0.001, (to skyCl{i}AhaAddr from skyCl{i}AhaAddr : SkyhookStartCmd), 0]")
    for i in [1, 2]:
        L(f"    [0.20, (to wtCl{i}ClientAddr from wtCl{i}ClientAddr : WtStartCmd), 0]")
    L("")
    
    for i in [3, 4]:
        L(f"    [1.0 + genRandomX(j, 0.0, 0.0001), (to skyCl{i}UmacAddr from skyCl{i}CmacAddr : actionR(\"ok\")), 0]")
        L(f"    [1.0 + genRandomX(s s j, 0.0, 0.0001), (to skyCl{i}UmasAddr from skyCl{i}UmasAddr : actionR(\"ok\")), 0]")
    
    for i in [7, 8]:
        L(f"    [1.0 + genRandomX(j, 0.0, 0.0001), (to iodCl{i}SendAppAddr : start), 0]")
        L(f"    [1.0 + genRandomX(j, 0.0, 0.0001), (to iodCl{i}RcvAppAddr : start), 0]")
    
    i = 9
    L(f"    [1.0 + genRandomX(j, 0.0, 0.0001), (to masCl{i}UmacAddr from masCl{i}CmacAddr : actionR(\"ok\")), 0]")
    L(f"    [1.0 + genRandomX(s s j, 0.0, 0.0001), (to masCl{i}UmasAddr from masCl{i}UmasAddr : actionR(\"ok\")), 0]")
    L("")
    
    L("    --- IRC User Model Burst Delay Timers (staggered)")
    start_time = 20.0
    for i in [1, 2]:
        L(f"    [{start_time}, (to wtCl{i}UmAddr from wtCl{i}UmAddr : burstDelayTO), 0]")
        start_time += 1.0
    for i in [3, 4]:
        L(f"    [{start_time}, (to skyCl{i}UmAddr from skyCl{i}UmAddr : burstDelayTO), 0]")
        start_time += 1.0
    for i in [5, 6]:
        L(f"    [{start_time}, (to obfsCl{i}UmAddr from obfsCl{i}UmAddr : burstDelayTO), 0]")
        start_time += 1.0
    for i in [7, 8]:
        L(f"    [{start_time}, (to iodCl{i}UmAddr from iodCl{i}UmAddr : burstDelayTO), 0]")
        start_time += 1.0
    i = 9
    L(f"    [{start_time}, (to masCl{i}UmAddr from masCl{i}UmAddr : burstDelayTO), 0]")
    L("")
    ...
    for inst in tgen_instances:
        bn = inst.base_name
        L(f"    [{tgen_start} + genRandomX(j, 0.0, 0.0001), (to {bn}UmAddr from {bn}UmAddr : burstDelayTO), 0]")
    L("  .")
    L("")
    ...
    all_clients = []
    for i in [1, 2]:
        all_clients.append(f"wtCl{i}IrcAddr")
    for i in [3, 4]:
        all_clients.append(f"skyCl{i}IrcAddr")
    for i in [5, 6]:
        all_clients.append(f"obfsCl{i}IrcAddr")
    for i in [7, 8]:
        all_clients.append(f"iodCl{i}IrcAddr")
...
if __name__ == "__main__":
    yaml_file = sys.argv[1] if len(sys.argv) > 1 else os.path.join(OUT_DIR, "pwnd_cp3_scenario_1.yaml")
    
    print(f"Parsing scenario YAML from: {yaml_file}")
    duration, networks, net_id_map, net_short, loss_profiles, hcs_nodes, hcs_profiles_by_channel, tgen_defs = parse_scenario_yaml(yaml_file)
    ...
    addr_content = gen_addresses_file(tgen_instances, net_id_map)
    addr_path = os.path.join(OUT_DIR, "scenario1_addresses.maude")
    with open(addr_path, "w") as f:
        f.write(addr_content)
    print(f"\nWrote {addr_path} ({len(addr_content.splitlines())} lines)")
    
    # Generate main file
    main_content = gen_main_file(tgen_instances, networks, loss_profiles, hcs_profiles_by_channel, duration)
    main_path = os.path.join(OUT_DIR, "scenario1.maude")
