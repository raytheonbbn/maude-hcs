...
def gen_main_file(tgen_instances, networks, loss_profiles, hcs_profiles_by_channel, duration):
    ...
    L("--- For SMC")
    L(f"sload {lib}/common/maude/visibility")
    L("")
    ...
    L("  eq hcsDelay  = 0. [owise] .")
    L("  eq tgenDelay = 30.0 [owise] .")
    L("")
    ...
    L("    < 'ns . 't1 . 'pwnd . 'com . root, a, 360000.0, iodCl7SrvNetClAddr >")
    L("    < 't2 . 'pwnd . 'com . root, ns, 360000.0, 'ns . 't2 . 'pwnd . 'com . root >")
    L("    < 'ns . 't2 . 'pwnd . 'com . root, a, 360000.0, iodCl8SrvNetClAddr >")
    ...
    for i in [7, 8]:
        ...
        L(f"    < 'pwnd . 'com . root, ns, 360000.0, 'ns . '{t_id} . 'pwnd . 'com . root >")
        L(f"    < 'ns . '{t_id} . 'pwnd . 'com . root, a, 360000.0, iodCl{i}SrvNetClAddr >")
        L(f"    < 'www0 . '{t_id} . 'pwnd . 'com . root, a, 360000.0, 2 . 0 . 1 . 2 >")
        L(f"    < 'www1 . '{t_id} . 'pwnd . 'com . root, a, 360000.0, 2 . 1 . 1 . 2 >")
        L(f"    < wildcard . '{t_id} . 'pwnd . 'com . root, txt, 360000.0, nullAddr > .")
        L("")
    ...
    L("    cacheEntry(< 'ns . 't1 . 'pwnd . 'com . root, a, 360000.0, iodCl7SrvNetClAddr >, 1)")
    L("    cacheEntry(< 't2 . 'pwnd . 'com . root, ns, 360000.0, 'ns . 't2 . 'pwnd . 'com . root >, 1)")
    L("    cacheEntry(< 'ns . 't2 . 'pwnd . 'com . root, a, 360000.0, iodCl8SrvNetClAddr >, 1)")
    for zone_label, dns_addr in [
        ("mas", "corpMasDnsAddr"), ("rt", "corpRtDnsAddr"),
        ("iod", "corpIodDnsAddr"), ("obfs", "corpObfsDnsAddr"),
        ("sky", "corpSkyDnsAddr"), ("serv", "servDnsAddr")
    ...
    L("  eq transport(corpIodDnsAddr)    = corpIodDnsAddr .")
    for i in [7, 8]:
    ...
    for idx, i in enumerate([7, 8]):
        ...
        L(f"  eq iodCl{i}Cl        = makeWClient(iodCl{i}ClientAddr, corpIodDnsAddr, '{t_id} . 'pwnd . 'com . root, a, 0.0) .")
...
if __name__ == "__main__":
    ...
    addr_path = os.path.join(OUT_DIR, "scenario1_addressesv2.maude")
    ...
    main_path = os.path.join(OUT_DIR, "scenario1v2.maude")
