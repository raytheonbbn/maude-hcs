...
```
This estimate is for the expercetd value of latency `E[latency]` (as indicated in `measure`).
We also include the probability of detect for the scalability estimated: this is computed as `discarded`/(`discarded`+`nsims`),
