...
def extract_data_for_query(json_data, query_name):
    """
    Extracts sorted wait values, means, and radii for a given query and n.
    """
    ...
    for key, val in json_data.items():
        w_vals.append(val['cli_wait_time'])
        q_data = val['scheck']['queries'][query_idx]
        means.append(q_data['mean'])
        radii.append(q_data['radius'])

    # Sort the lists based on w to ensure lines plot correctly
    sorted_indices = sorted(range(len(w_vals)), key=lambda i: w_vals[i], reverse=True)
    w_vals = [w_vals[i] for i in sorted_indices]
    means = [means[i] for i in sorted_indices]
    radii = [radii[i] for i in sorted_indices]

    return w_vals, means, radii
...
def main():
    ...
    output_dir = os.path.join(input_dir, "plots_wait")
    ...
    for filename in os.listdir(input_dir):
        ...
        match = re.search(r'cp2_scenario_(\d+)_cli_wait', filename)
        if match:
    ...
    # ==========================================
    # Plot Set 1: HCS metrics vs wait time (All scenarios per query)
    # ==========================================
    for q_name in set1_queries:
        ...
        for scen_id in sorted(scenarios.keys()):
            ...
            w_vals, means, radii = extract_data_for_query(hcs_data, q_name)
            if not w_vals:
                continue

            plt.errorbar(w_vals, means, yerr=radii, marker='o',
                         capsize=5, label=f"Scenario {scen_id}", linewidth=2)
            plotted = True

        if plotted:
            plt.title(f"{q_name} {units(q_name)}")
            plt.xlabel("Client Wait Time")
            plt.ylabel(f"Mean Value")
            plt.grid(True, linestyle='--', alpha=0.7)
            plt.legend()

            out_filename = os.path.join(output_dir, f"set1_{q_name}_wait.png")
    ...
    #             continue
    #
    #         w_tpr, mean_tpr, rad_tpr = extract_data_for_query(hcs_data, q_name)
    #         w_fpr, mean_fpr, rad_fpr = extract_data_for_query(ord_data, q_name)
    #
    #         tpr_dict = {k: (m, r) for k, m, r in zip(w_tpr, mean_tpr, rad_tpr)}
    #         fpr_dict = {k: (m, r) for k, m, r in zip(w_fpr, mean_fpr, rad_fpr)}
    #
    #         kl_w_vals = []
    #         kl_bounds = []
    #
    #         # Only compute for 'k' values present in both datasets
    #         common_ks = sorted(set(w_tpr) & set(w_fpr))
    #         for k in common_ks:
    #             t_m, t_r = tpr_dict[k]
    #             f_m, f_r = fpr_dict[k]
    #
    ...
    #                 print(f'{scen_id}:{q_name}:w={k} overlap  {t_m} +/- {t_r} <-> {f_m} +/- {f_r} ==> {kl_val}')
    #             else:
    #                 print(f'{scen_id}:{q_name}:w={k} -------  {t_m} +/- {t_r} <-> {f_m} +/- {f_r} ==> {kl_val}')
    #
    #         if kl_w_vals:
    #             plt.plot(kl_w_vals, kl_bounds, marker='s',
    ...
    #     plt.close()
        ...
        for q_name in set2_queries:
            ...
            for scen_id in sorted(scenarios.keys()):
                ...
                k_tpr, mean_tpr, rad_tpr = extract_data_for_query(hcs_data, q_name)
                k_fpr, mean_fpr, rad_fpr = extract_data_for_query(ord_data, q_name)
            ...
            if has_inf:
                ...
                ax2.set_xlabel("Client Wait Time")
