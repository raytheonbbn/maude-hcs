...
def smc(param_space, scenario_path, scenario_id, smc_path:Path, tgenonly=False):
    ...
    for p in pairs:
        ...
        if tgenonly:
            ...
            subprocess.run([ "sed", "-i", r"s/slimit[[:space:]]*=[[:space:]]*6000\.0/slimit = 600.0/", tgenonly_fn ], check=True)
        else:
        ...
        if tgenonly:
            scheck_cmd = ["maude-hcs", "scheck", "--test=" + scenario_path + "/" + generated_test_path + "_tgenonly.maude", "--query=" + str(smc_path) + "/cp2_eval_" + generated_test_path + ".quatex", "--format", "json", "-j", "0", "--nsims", "300-300", "--dump", output_dir + "/cp2_scenario_" + str(scenario_id) + "_" + experiment + generated_test_path + "_tgenonly.dat"]
        else:
            scheck_cmd = ["maude-hcs", "scheck", "--test=" + scenario_path + "/" + generated_test_path + ".maude", "--query=" + str(smc_path) + "/cp2_eval_" + generated_test_path + ".quatex", "--format", "json", "-j", "0", "--nsims", "300-300", "--dump", output_dir + "/cp2_scenario_" + str(scenario_id) + "_" + experiment + generated_test_path + ".dat"]
