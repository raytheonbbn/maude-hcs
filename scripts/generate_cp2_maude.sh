...
# Invoke with PWND2DIR as first argument
PWND2DIR=${1:-".."}
...
do
    ...
    maude-hcs --verbose --protocol=mastodon markov \
        --json-dir="$PWND2DIR/pwnd-cp2/cp2_scenarios/cp2_scenario_$i/actions/" \
        --maude-dir="./maude_hcs/lib/raceboat/maude/mastodonprofiles/"

    # Command 2: Generate maude init config
    maude-hcs --verbose generate \
        --yml-filename="$PWND2DIR/pwnd-cp2/cp2_scenarios/cp2_scenario_$i/cp2_scenario_$i.yml" \
        --model=prob \
